...
def listnotes(request, user_id=None):
    ...
    if request.method == 'GET':
        ...
        listofnotes = Notesmodel.objects.filter(user_id=user_id).order_by('due_date').values('title','category','description','due_date')
...
def sites(request, user_id=None):
    ...
    if request.method == 'POST':
        ...
        note.due_date = received_json_data.get("due_date")
        note.title = received_json_data.get('title')
        note.category = received_json_data.get('category')
        try:
            note.save()
        except:
